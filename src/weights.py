...
def compute_structural_weights_batched(A, n_nodes, batch_size=512,
                                       method='ppr', alpha=0.15,
                                       num_iterations=10, device=None):
    """
    Compute structural weights for a batch of anchor nodes.

    For large graphs (N > 5000), computes weights for B anchor nodes at a time
    ...
    Returns:
        callable: Function that takes anchor indices and returns their weight rows
    """
    # Precompute full PPR matrix
    W_s_full = compute_structural_weights(A, n_nodes, method, alpha, num_iterations, device)

    def get_batch_weights(anchor_indices):
        """Get weight rows for specified anchor nodes."""
        return W_s_full[anchor_indices]

    return get_batch_weights


def compute_attribute_weights_batched(X, batch_size=512, device=None):
    """
    Compute attribute weights for a batch of anchor nodes.
    ...
    Returns:
        callable: Function that takes anchor indices and returns their weight rows
    """
    W_a_full = compute_attribute_weights(X)

    def get_batch_weights(anchor_indices):
        """Get weight rows for specified anchor nodes."""
        return W_a_full[anchor_indices]

    return get_batch_weights


def slice_weight_matrix(W, batch_indices):
    """
    Slice weight matrix for a batch of nodes.

    Args:
        W (torch.Tensor): Weight matrix, shape (N, N)
        batch_indices (torch.Tensor): Node indices for batch, shape (B,)

    Returns:
        torch.Tensor: Sliced weight matrix, shape (B, B)
    """
    # Get rows for batch nodes
    W_rows = W[batch_indices]  # shape (B, N)

    # Get columns for batch nodes
    W_batch = W_rows[:, batch_indices]  # shape (B, B)

    # Row-normalize
    row_sums = W_batch.sum(dim=1, keepdim=True)
    row_sums = torch.clamp(row_sums, min=1e-8)
    W_batch = W_batch / row_sums
